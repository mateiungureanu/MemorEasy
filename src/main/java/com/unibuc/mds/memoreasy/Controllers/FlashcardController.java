...
public class FlashcardController implements Initializable {
    ...
    public TextArea getFrontTextArea() {
        return frontTextArea;
    }

    public TextArea getBackTextArea() {
        return backTextArea;
    }
    ...
    public void flipCard() {
        ...
        Timeline firstHalf = new Timeline(new KeyFrame(Duration.millis(300), new KeyValue(card.rotateProperty(), (startAngle + endAngle) / 2)));

        firstHalf.setOnFinished(e -> {
            front.setVisible(!showingFront);
            back.setVisible(showingFront);

            Timeline secondHalf = new Timeline(new KeyFrame(Duration.millis(300), new KeyValue(card.rotateProperty(), endAngle)));
            secondHalf.play();
