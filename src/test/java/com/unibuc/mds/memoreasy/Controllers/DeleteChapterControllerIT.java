...
class DeleteChapterControllerIT extends ApplicationTest {
    ...
    void shouldShowDeleteButton() {
        Button deleteButton = lookup("#buttonDelete").queryButton();
        assertThat(deleteButton).isVisible();
    }

    @Test
    void shouldDeleteChapterFromDatabase() throws Exception {
        Button deleteButton = lookup("#buttonDelete").queryButton();
        ...
        try (Connection con = com.unibuc.mds.memoreasy.Utils.DatabaseUtils.getConnection()) {
            PreparedStatement ps = con.prepareStatement("SELECT * FROM chapter WHERE id_chapter = ?");
    ...
    @Test
    void shouldNotDeleteChapterWhenClickingCancel() throws Exception {
